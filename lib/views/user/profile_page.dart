...
import 'package:flutter_bengkelin/config/pref.dart'; // Untuk Session()
import 'package:flutter_bengkelin/model/user_model.dart'; // Untuk UserModel
import 'package:flutter_bengkelin/viewmodel/profile_viewmodel.dart'; // Untuk ProfileViewmodel
import 'package:flutter_bengkelin/views/user/edit_profile.dart'; // Untuk navigasi ke EditProfilePage
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void initState() {
    super.initState();
    getUserProfile(); // Panggil metode untuk memuat data profil saat inisialisasi
  }

  // Metode untuk memuat data profil menggunakan ProfileViewmodel
  getUserProfile() async {
    setState(() {
      _isLoading = true; // Mulai loading
      _users = null; // Reset data pengguna saat mulai memuat ulang
    });

    try {
      final respValue = await ProfileViewmodel()
          .getUserProfile(); // Panggil viewmodel

      if (!mounted) return; // Pastikan widget masih aktif sebelum setState

      // Menggunakan respValue.code dan respValue.status untuk pengecekan sukses
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed:
                        getUserProfile, // Tombol "Muat Ulang" untuk coba lagi
                  ...
                  children: [
                    ...
                    _users!.profilePicture != null &&
                        ...
                        : CircleAvatar(
                            radius: 60,
                            backgroundColor: Colors.grey[200],
                            // Menggunakan Icon default user atau gambar asset default
                            child: Image.asset(
                              'assets/profile1.png', // Pastikan path ini benar di pubspec.yaml
                              fit: BoxFit.cover,
                              errorBuilder: (context, error, stackTrace) {
                                // Fallback jika gambar asset juga gagal dimuat
                                return const Icon(
                                  Icons.person,
                                  size: 60,
                                  color: Colors.grey,
                    ...
                    Text(
                      _users!.name, // Menggunakan data dari _users
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          // Teruskan data pengguna yang ada ke halaman edit
                          final result = await Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) => EditProfilePage(
                                ...
                                userPhotoUrl: _users!
                                    .profilePictureUrl, // Teruskan URL gambar ke halaman edit
                              ),
                            ),
                          );

                          if (result == true) {
                            // Asumsi EditProfilePage mengembalikan true jika berhasil diupdate
                            debugPrint('Profile updated. Reloading data...');
                            getUserProfile(); // Muat ulang data setelah edit
                          }
                        },
