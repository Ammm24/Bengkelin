...
import 'package:flutter_bengkelin/config/pref.dart';
...
=======
      ...
      if (rest.data is Map<String, dynamic>) {
        ...
        return Resp(code: rest.statusCode ?? 200, message: 'Unexpected response format for POST', data: rest.data);
      }
    } on DioException catch (e) {
      debugPrint('DioException (postApi) - URL: ${e.requestOptions.uri}');
      debugPrint('DioException (postApi) - Response status: ${e.response?.statusCode}');
      ...
      if (e.response != null) {
        ...
        return Resp(code: 500, message: e.message ?? 'Unknown network error', error: e.toString());
      }
    } catch (e) {
      debugPrint('Unexpected error (postApi): $e');
      return Resp(code: 500, message: 'An unexpected error occurred', error: e.toString());
    }
  }
...
=======
      if (restValue.data is Map<String, dynamic>) {
        ...
        return Resp(code: restValue.statusCode ?? 200, message: 'Unexpected response format for POST with headers', data: restValue.data);
      }
    } on DioException catch (e) {
      debugPrint('DioException (postApiWithHeaders) - URL: ${e.requestOptions.uri}');
      debugPrint('DioException (postApiWithHeaders) - Response status: ${e.response?.statusCode}');
      debugPrint('DioException (postApiWithHeaders) - Response data: ${e.response?.data}');
      if (e.response?.statusCode == 401) {
        await Session().logout();
        // Anda mungkin ingin menavigasi ke halaman login di sini
        // navigatorKey.currentState?.pushNamedAndRemoveUntil('/', (route) => false,);
        return Resp(code: 401, message: 'Unauthorized', error: e.response?.data);
      }
      if (e.response != null) {
        if (e.response!.data is Map<String, dynamic>) {
          return Resp.fromJson(e.response!.data);
...
=======
      ...
      if (restGet.data is Map<String, dynamic>) {
        ...
        return Resp(code: restGet.statusCode ?? 200, success: true, data: restGet.data);
      } else {
        // Handle tipe respons lain yang tidak terduga
        return Resp(code: restGet.statusCode ?? 500, message: 'Unexpected API response format', error: restGet.data.toString());
      }
    } on DioException catch (e) {
      debugPrint('DioException (getApi) - URL: ${e.requestOptions.uri}');
      debugPrint('DioException (getApi) - Response status: ${e.response?.statusCode}');
...
=======
      ...
      if (restValue.data is Map<String, dynamic>) {
        ...
        return Resp(code: restValue.statusCode ?? 200, message: 'Unexpected response format for PUT', data: restValue.data);
      }
    } on DioException catch (e) {
      debugPrint('DioException (putApi) - URL: ${e.requestOptions.uri}');
      debugPrint('DioException (putApi) - Response status: ${e.response?.statusCode}');
  ...
  static Future<Resp> putApiWithHeaders(
    ...
    try {
      ...
      if (restValue.data is Map<String, dynamic>) {
        ...
        return Resp(code: restValue.statusCode ?? 200, message: 'Unexpected response format for PUT with headers', data: restValue.data);
      }
    } on DioException catch (e) {
      debugPrint('DioException (putApiWithHeaders) - URL: ${e.requestOptions.uri}');
      debugPrint('DioException (putApiWithHeaders) - Response status: ${e.response?.statusCode}');
      debugPrint('DioException (putApiWithHeaders) - Response data: ${e.response?.data}');
      if (e.response != null) {
        if (e.response!.data is Map<String, dynamic>) {
          return Resp.fromJson(e.response!.data);
        } else if (e.response!.data is String) {
